...
const TIPO_CFG: Record<TipoCliente, { bg: string; text: string; dot: string }> = {
  farmacia:     { bg: 'bg-blue-50',    text: 'text-blue-700',   dot: '#2563eb' },
  clinica:      { bg: 'bg-violet-50',  text: 'text-violet-700', dot: '#7c3aed' },
  centro_salud: { bg: 'bg-emerald-50', text: 'text-emerald-700',dot: '#059669' },
  consultorio:  { bg: 'bg-cyan-50',    text: 'text-cyan-700',   dot: '#0891b2' },
}
...
function Drawer({ cliente, onClose, onToggleActivo, onEdit }: {
  ...
  const fmtFecha = (iso: string) =>
    new Date(iso).toLocaleDateString(lang === 'en' ? 'en-US' : 'es-BO', { day: '2-digit', month: 'long', year: 'numeric' })
      ...
      >
        ...
        <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1">{t('clientes.detailTitle')}</p>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {/* Zona */}
          <Section title={t('common.zone')}>
            <InfoRow label={t('common.zone')} value={
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold" style={{ color: cliente.zona_color }}>
                ...
                {tZona(cliente.zona_nombre)}
              </span>
            } />
            <InfoRow label={t('clientes.address')} value={cliente.direccion} />
          </Section>

          {/* Contacto */}
          <Section title={t('clientes.contact')}>
            <InfoRow label={t('clientes.responsible')} value={cliente.contacto_nombre} />
            <InfoRow label={t('clientes.phone')}       value={cliente.telefono} />
            <InfoRow label={t('clientes.email')}       value={cliente.email} />
          </Section>

          {/* Comercial */}
          <Section title={t('clientes.commercial')}>
            <InfoRow label={t('clientes.nit')}         value={cliente.nit} />
            <InfoRow label={t('clientes.creditLimit')} value={fmtBs(cliente.credito_limite)} />
            <InfoRow label={t('clientes.memberSince')} value={fmtFecha(cliente.created_at)} />
          </Section>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          ...
          >
            ...
            {saving ? t('common.saving') : cliente.activo ? t('clientes.suspend') : t('clientes.activate')}
          </button>
        </div>
        )}
      </aside>
...
function NuevoClienteModal({ onClose, onSuccess }: {
  ...
  const handleSubmit = async () => {
    if (!form.nombre || !form.zona_id || !form.direccion) {
      setErr(t('clientes.reqError')); return
    }
    setErr(null); setSaving(true)
    try {
  ...
  return (
    <>
      ...
      <div className="fixed z-50 bg-white rounded-2xl shadow-2xl border border-slate-100 w-[calc(100vw-2rem)] max-w-lg p-6 max-h-[90vh] overflow-y-auto"
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-0.5">{t('clientes.modalEyebrow')}</p>
            <h2 className="text-lg font-extrabold text-slate-900">{t('clientes.new')}</h2>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        ...
        <div className="space-y-3">
          <div>
            <label className={LBL}>{t('clientes.fieldName')}</label>
            <input type="text" value={form.nombre} onChange={set('nombre')} placeholder={t('clientes.namePlaceholder')} className={INP} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={LBL}>{t('clientes.fieldType')}</label>
              <select value={form.tipo} onChange={set('tipo')} className={INP}>
                <option value="farmacia">{t('tipo.farmacia')}</option>
                <option value="clinica">{t('tipo.clinica')}</option>
                <option value="centro_salud">{t('tipo.centro_salud')}</option>
                <option value="consultorio">{t('tipo.consultorio')}</option>
              </select>
            </div>
            <div>
              <label className={LBL}>{t('clientes.fieldZone')}</label>
          ...
          <div>
            <label className={LBL}>{t('clientes.fieldAddress')}</label>
            <input type="text" value={form.direccion} onChange={set('direccion')} placeholder={t('clientes.addressPlaceholder')} className={INP} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={LBL}>{t('clientes.fieldPhone')}</label>
              <input type="tel" value={form.telefono} onChange={set('telefono')} placeholder={t('clientes.phonePlaceholder')} className={INP} />
            </div>
            <div>
              <label className={LBL}>{t('clientes.fieldEmail')}</label>
              <input type="email" value={form.email} onChange={set('email')} placeholder={t('clientes.emailPlaceholder')} className={INP} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={LBL}>{t('clientes.fieldContact')}</label>
              <input type="text" value={form.contacto_nombre} onChange={set('contacto_nombre')} placeholder={t('clientes.contactPlaceholder')} className={INP} />
            </div>
            <div>
              <label className={LBL}>{t('clientes.fieldCredit')}</label>
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button onClick={handleSubmit} disabled={saving || !zonas.length}
            ...
            style={{ background: '#1e40af' }}>
            {saving && <Spinner />}
            {saving ? t('common.saving') : t('clientes.create')}
          </button>
        </div>
      </div>
    </>
...
export default function Clientes() {
  ...
  const handleToggleActivo = async (newActivo: boolean) => {
    ...
    setSelected(updated)
    showToast('success', newActivo ? t('clientes.toastActivated') : t('clientes.toastSuspended'))
  }
  ...
  const hoy = new Date().toLocaleDateString(lang === 'en' ? 'en-US' : 'es-BO', {
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 space-y-5 max-w-7xl w-full mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3">
          {loading ? (
            ...
            <>
              <StatCard label={t('clientes.statTotal')} value={stats.total} color="#1e40af" />
              <StatCard label={t('clientes.statActive')} value={stats.activos} color="#15803d" />
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {TIPOS.map(tp => (
              <FilterPill
                key={tp}
                active={tipoFilter === tp}
                onClick={() => setTipoFilter(tp)}
                color={TIPO_CFG[tp].dot}
              >
                {t('tipo.' + tp)}
              </FilterPill>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 py-3.5 border-b border-slate-100 flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500">
              {loading ? '…' : `${filtered.length} ${filtered.length !== 1 ? t('clientes.countMany') : t('clientes.countOne')}`}
              {filtered.length !== clientes.length && !loading && (
                <span className="ml-1 text-slate-400">{t('common.of')} {clientes.length} {t('common.totalSuffix')}</span>
              )}
            </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {[t('clientes.colClient'), t('clientes.colType'), t('clientes.colZone'), t('clientes.colPhone'), t('clientes.colContact'), t('clientes.colState'), ''].map((h, idx) => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-16 text-center">
                      <p className="text-slate-400 text-sm">{t('clientes.empty')}</p>
