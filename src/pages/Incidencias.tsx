...
function RegistrarIncidenciaModal({ onClose, onSuccess }: {
  ...
  const handleGuardar = async () => {
    if (!pedidoId)      { setErr(t('incidencias.modal.errOrder')); return }
    if (!descripcion.trim()) { setErr(t('incidencias.modal.errDesc')); return }
    setErr(null)
    setSaving(true)
    try {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-0.5">{t('incidencias.modal.eyebrow')}</p>
            <h2 className="text-lg font-extrabold text-slate-900">{t('incidencias.modal.title')}</h2>
          </div>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('incidencias.modal.order')}</label>
          {loadingPed ? (
            <div className="w-full h-10 bg-slate-100 rounded-xl animate-pulse" />
          ) : pedidos.length === 0 ? (
            <p className="text-sm text-slate-400">{t('incidencias.modal.noOrders')}</p>
          ) : (
            <select
              value={pedidoId}
        ...
        {/* Tipo */}
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('incidencias.modal.type')}</label>
          <select
            value={tipo}
          ...
          >
            {TIPOS_INCIDENCIA.map(v => (
              <option key={v} value={v}>{t('incidencias.tipo.'+v)}</option>
            ))}
          </select>
        </div>
        ...
        {/* Descripción */}
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t('incidencias.modal.description')}</label>
          <textarea
            value={descripcion}
            onChange={e => setDescripcion(e.target.value)}
            rows={3}
            placeholder={t('incidencias.modal.phDesc')}
        ...
        <div className="flex gap-3 pt-1">
          ...
          >
            ...
            {saving ? t('common.saving') : t('incidencias.modal.submit')}
          </button>
        </div>
      </div>
    </>
...
function EstadoBadgeInc({ estado }: { estado: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${resuelta ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
      ...
      {resuelta ? t('incidencias.status.resolved') : t('incidencias.status.pending')}
    </span>
  )
}
  ...
  return (
    <>
      <header className="sticky top-14 lg:top-0 z-10 bg-white border-b border-slate-100 px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-extrabold tracking-tight" style={{ color: '#0f172a' }}>{t('incidencias.title')}</h1>
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 space-y-5 max-w-7xl w-full mx-auto">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          {loading ? Array.from({length:3}).map((_,i)=><Skeleton key={i} className="h-16" />) : (
            <>
              <StatCard label={t('incidencias.stat.total')} value={incidencias.length} color="#1e40af" />
              <StatCard label={t('incidencias.stat.pending')} value={pendientes} color={pendientes>0?'#dc2626':'#64748b'} />
              <StatCard label={t('incidencias.stat.resolved')} value={incidencias.length-pendientes} color="#15803d" />
            </>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1 max-w-xs">
            ...
            <input type="text" placeholder={t('incidencias.search')} value={search} onChange={e=>setSearch(e.target.value)}
          ...
          <div className="flex gap-1.5">
            <FilterPill active={estadoFilter==='todos'} onClick={()=>setEstadoFilter('todos')}>{t('incidencias.filter.all')}</FilterPill>
            <FilterPill active={estadoFilter==='pendiente'} onClick={()=>setEstadoFilter('pendiente')} color="#d97706">{t('incidencias.filter.pending')}</FilterPill>
            <FilterPill active={estadoFilter==='resuelta'} onClick={()=>setEstadoFilter('resuelta')} color="#16a34a">{t('incidencias.filter.resolved')}</FilterPill>
          </div>
        </div>

        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 py-3.5 border-b border-slate-100">
            <p className="text-xs font-semibold text-slate-500">{loading ? '…' : `${filtered.length} ${filtered.length!==1?t('incidencias.count.many'):t('incidencias.count.one')}`}</p>
          </div>
          <div className="hidden md:block overflow-x-auto">
            {!loading && incidencias.length === 0 ? (
              <div className="py-20 text-center space-y-3">
                ...
                <p className="text-base font-bold text-slate-700">{t('incidencias.empty.title')}</p>
                <p className="text-sm text-slate-400">{t('incidencias.empty.subtitle')}</p>
              </div>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    {[t('incidencias.col.type'),t('incidencias.col.order'),t('incidencias.col.courier'),t('incidencias.col.description'),t('incidencias.col.status'),t('incidencias.col.date'),''].map(h=>(
      ...
      {showModal && (
        <RegistrarIncidenciaModal
          ...
          onSuccess={() => {
            setShowModal(false)
            loadIncidencias()
            showToast('success', t('incidencias.toast.created'))
          }}
        />
      )}
        ...
        <>
          ...
          <aside className="fixed right-0 top-0 bottom-0 w-full sm:w-96 bg-white z-40 shadow-2xl flex flex-col slide-in-right">
            <div className="px-6 pt-6 pb-5 border-b border-slate-100 flex items-start justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1">{t('incidencias.drawer.eyebrow')}</p>
                <p className="font-bold text-slate-900">{t('incidencias.tipo.'+selected.tipo)}</p>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              {selected.pedido_codigo && <InfoBlock label={t('incidencias.drawer.order')} value={<span className="font-data text-blue-700">{selected.pedido_codigo}</span>} />}
              {selected.repartidor_nombre && <InfoBlock label={t('incidencias.drawer.courier')} value={selected.repartidor_nombre} />}
              <InfoBlock label={t('incidencias.drawer.description')} value={selected.descripcion} />
